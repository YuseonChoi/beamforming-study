...
def get_3dim_spectrum(wav_name, channel_vec, start_point, stop_point, frame, shift, fftl):
    ...
    st = 0
    ed = frame
    number_of_frame = np.int32((len(samples) - frame) /  shift)
    spectrums = np.zeros((len(channel_vec), number_of_frame, np.int32(fftl / 2) + 1), dtype=np.complex64)
    for ii in range(0, number_of_frame):
        multi_signal_spectrum = fft(dump_wav[:, st:ed], n=fftl, axis=1)[:, 0:np.int32(fftl / 2) + 1] # channel * number_of_bin        
...
def get_3dim_spectrum_from_data(wav_data, frame, shift, fftl):
    ...
    st = 0
    ed = frame
    number_of_frame = np.int32((len_sample - frame) /  shift)
    spectrums = np.zeros((len_channel_vec, number_of_frame, np.int32(fftl / 2) + 1), dtype=np.complex64)
    for ii in range(0, number_of_frame):       
        multi_signal_spectrum = fft(dump_wav[:, st:ed], n=fftl, axis=1)[:, 0:np.int32(fftl / 2) + 1] # channel * number_of_bin        
        spectrums[:, ii, :] = multi_signal_spectrum
        st = st + shift
...
def spec2wav(spectrogram, sampling_frequency, fftl, frame_len, shift_len):
    ...
    for ii in range(0, n_of_frame):
        ...
        cut_data[0:np.int32(fftl / 2) + 1] = half_spec.T   
        cut_data[np.int32(fftl / 2) + 1:] =  np.flip(np.conjugate(half_spec[1:np.int32(fftl / 2)]), axis=0)
